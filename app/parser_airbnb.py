...
def find_data_room(driver, country, time_correction, price_min, price_max):
    print("info: Getting data (id, url) of objects in the search")
    html = driver.page_source
    nand = BeautifulSoup(html, 'lxml')
    quick_sleep(2, 3)
    ...
    for el in nand.find_all("div", {"data-testid": "card-container"}):
        ...
        if data_room is not None:
            ...
            print(f"Added {id} and url to the database")


# price_min, price_max

        data_position = asyncio.run(get_position(id))
        if data_room is not None:

        # Preparing data for the room - Готовим данные 
        price_data = {"date": list_date_update, "price_min": price_min, "price_max": price_max}
        asyncio.run(update_position(1, price_data))
        print(f"Added min - max to the database")
